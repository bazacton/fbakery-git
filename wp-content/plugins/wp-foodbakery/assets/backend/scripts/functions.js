...
function foodbakery_section_background_settings_toggle(id, rand_no) {
    ...
    if (id == "no-image") {
        ...
        jQuery(".section-gallery-slider-" + rand_no).hide();
    ...
    } else if (id == "section-gallery-slider") {
        jQuery(".section-slider-" + rand_no).hide();
        jQuery(".section-custom-slider-" + rand_no).hide();
        jQuery(".section-background-video-" + rand_no).hide();
        jQuery(".section-custom-background-image-" + rand_no).hide();
        jQuery(".section-gallery-slider-" + rand_no).show();
    } else {
        jQuery(".section-custom-background-image-" + rand_no).hide();
        jQuery(".section-slider-" + rand_no).hide();
        jQuery(".section-custom-slider-" + rand_no).hide();
...
jQuery(function ($) {
    ...
    jQuery('.add_gallery_plugin').on('click', 'input', function (event) {
        var $el = jQuery(this).closest('.add_gallery_plugin');
...
jQuery(document).on("change", ".publisher_profile_type", function () {
    ...
    jQuery(".condition-fields").hide();
    jQuery("."+field_type+"-fields").show();
...
jQuery(document).on("click", ".foodbakery_add_deal", function () {
    var restaurant_id   = jQuery(this).data('restaurant_id');
    jQuery.ajax({
        type: "POST",
        url: foodbakery_globals.ajax_url,
        data: 'action=foodbakery_add_deal&restaurant_id='+ restaurant_id,
        success: function (response) {
...
jQuery(document).on("click", ".submit-deal-form", function () {
    var deal_name = jQuery("#deal_name").val();
    var deal_price = jQuery("#deal_price").val();
    var deal_serving = jQuery("#deal_serving").val();
    var deal_image = jQuery("#foodbakery_deal_image_rand").val();
    var foodbakery_deal_menu_items = jQuery("#foodbakery_deal_menu_items").val();
    jQuery.ajax({
        type: "POST",
        url: foodbakery_globals.ajax_url,
        data: 'action=foodbakery_add_deal_form&deal_name='+ deal_name +'&deal_price='+ deal_price +'&deal_image='+ deal_image +'&deal_serving='+deal_serving+'&foodbakery_deal_menu_items='+foodbakery_deal_menu_items,
        success: function (response) {
...
jQuery(document).on('change', '#foodbakery_var_menu_style', function () {
    var menu_style = jQuery(this).val();
    jQuery("#foodbakery-fancy-many-fields").hide();
    if(menu_style == 'fancy'){
        jQuery("#foodbakery-fancy-many-fields").show();
    }
});


jQuery(document).on('click', '.delete_browse_image', function () {
    console.log('test');
    jQuery(this).closest('.icon-image-browse-field').find('.browse-icon').show();
    jQuery(this).closest('.icon-image-browse-field').find('.page-wrap').hide();
    if( jQuery("#foodbakery_listing_term_image_rand").length > 0){
        jQuery("#foodbakery_listing_term_image_rand").val('');
    }
    
    
});
jQuery(document).ready(function (e) {
    jQuery(document).on('click', '.foodbakery-verify-purchase-code', function () {
       var thisObj = jQuery(this);
       var foodbakery_purchase_code = jQuery('input[name="foodbakery_purchase_code"]').val();
       jQuery(".purchase-code-box-area").html('<div class="foodbakery-verfy-loader"><i class="item-status processing" style="width: 75px;height: 75px;"></i></div>');
       jQuery.ajax({
            type: "POST",
            url: foodbakery_globals.ajax_url,
            //dataType: "json",
            data: 'action=foodbakery_verify_purchase_code&foodbakery_purchase_code='+ foodbakery_purchase_code,
            success: function (response) {
                console.log(response);
...
jQuery(document).on('click', '.foodbakery-dereg-btn', function () {
    jQuery(this).prepend('<i class="item-status processing" style="width: 15px;height: 15px;margin: 2px 10px 0 0;float: left;"></i>');
    jQuery(this).removeClass('foodbakery-dereg-btn');
    jQuery.ajax({
        type: "POST",
        url: foodbakery_globals.ajax_url,
        dataType: "json",
        data: 'action=foodbakery_deregister_purchasecode',
        success: function (response) {
